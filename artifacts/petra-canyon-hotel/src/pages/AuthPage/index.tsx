...
const copyByLanguage = {
  ...
  ar: {
    ...
    loginBody: "سجل الدخول حتى تستخدم بياناتك المحفوظة بسرعة داخل طلبات الحجز.",
    signupTitle: "إنشاء حساب",
    signupBody: "أنشئ حساباً بسيطاً ليتم حفظ اسمك وبريدك ورقمك واستخدامها مباشرة داخل طلبات الحجز.",
    fullName: "الاسم الكامل",
    email: "البريد الإلكتروني",
    loginIdentifier: "البريد الإلكتروني أو رقم الهاتف",
  ...
  fr: {
    ...
    loginBody: "Connectez-vous pour utiliser plus rapidement vos informations dans les demandes de réservation.",
    signupTitle: "Créer un compte",
    signupBody: "Créez un compte simple pour enregistrer votre nom, votre e-mail et votre téléphone.",
    fullName: "Nom complet",
    email: "E-mail",
    loginIdentifier: "E-mail ou téléphone",
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (mode === "login") {
      ...
      if (result.user?.isAdmin) {
        window.sessionStorage.setItem("petra-admin-welcome", "pending");
        navigate("/admin");
        return;
      }
      navigate("/account");
        ...
        <div className="relative container mx-auto px-4 pb-16 sm:px-6 sm:pb-20">
          <div className="mx-auto grid max-w-5xl gap-8 lg:grid-cols-[0.95fr_1.05fr]">
            <aside className="rounded-[2rem] border border-primary/10 bg-[linear-gradient(145deg,rgba(61,40,32,0.96),rgba(127,79,60,0.86))] p-8 text-white shadow-[0_24px_80px_rgba(47,26,20,0.2)]">
              ...
              <p className="mt-4 text-sm leading-7 text-white/78 sm:text-base">{mode === "login" ? copy.loginBody : copy.signupBody}</p>
            </aside>
            ...
            <div className="rounded-[2rem] border border-primary/10 bg-white/90 p-6 shadow-[0_20px_70px_rgba(43,27,21,0.08)] backdrop-blur-xl sm:p-8 lg:p-10">
              <form className="grid gap-5" onSubmit={handleSubmit}>
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-foreground">{mode === "login" ? copy.loginIdentifier : copy.email}</label>
                  <div className="relative">
                    <Mail className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-primary/70" />
                    <Input type={mode === "login" ? "text" : "email"} value={form.email} onChange={(e) => updateField("email", e.target.value)} required className="h-12 rounded-2xl border-primary/10 bg-background/70 pl-11" />
                  </div>
                </div>
